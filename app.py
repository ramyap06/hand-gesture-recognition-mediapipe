...
def _tasks_landmarks_to_solution(task_landmarks):
    """
    task_landmarks: list[NormalizedLandmark] from Tasks
    returns: object with .landmark[i].x/.y/.z
    """
    return _FakeHandLandmarks(
        [_FakeLandmark(lm.x, lm.y, getattr(lm, "z", 0.0)) for lm in task_landmarks]
    )


def _tasks_handedness_to_solution(handedness_list):
    """
    handedness_list: list[Category] from Tasks result.handedness[i]
    returns: object with .classification[0].label
    """
    label = handedness_list[0].category_name if handedness_list else ""
    return _FakeHandedness(label)
...
def main():
    ...
    # Create landmarker and run loop ########################################
    with vision.HandLandmarker.create_from_options(options) as hand_landmarker:
        while True:
            ...
            # Detection implementation (Tasks) ################################
            ...
            if result.hand_landmarks:
                for task_lms, task_handedness in zip(result.hand_landmarks, result.handedness):
                    hand_landmarks = _tasks_landmarks_to_solution(task_lms)
                    handedness = _tasks_handedness_to_solution(task_handedness)
...
def calc_bounding_rect(image, landmarks):
    ...
    landmark_array = np.array(pts, dtype=np.int32)
...
def draw_landmarks(image, landmark_point):
    ...
    for index, landmark in enumerate(landmark_point):
        r = 8 if index in (4, 8, 12, 16, 20) else 5
